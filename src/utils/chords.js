...
export function transposeInlineChords(text, steps) {
  ...
  return text.replace(
    /\[([^\]]+)\]/g,
    (_, chord) => `[${transposeChord(chord.trim(), steps)}]`,
  );
}
...
function splitChordWithOptionalInversion(chord, inversion = "") {
  ...
  const suffix = suffixMatch
    ...
    : suffixWithSpaces.trim();
  const parsedInversion = suffixMatch
    ? normalizeInversionValue(suffixMatch[2])
    : "";
  const effectiveInversion =
    normalizeInversionValue(inversion) || parsedInversion;
...
export function getChordNoteIndexes(chord, inversion = "") {
  ...
  const reorderedIntervals = baseIntervals.map(
    (_, idx) => baseIntervals[(idx + inversionSteps) % baseIntervals.length],
  );

  const notes = reorderedIntervals.map(
...
function rotateArray(list, steps) {
  if (!Array.isArray(list) || list.length === 0 || !steps)
    return [...(list || [])];
...
export function getChordOrderedNoteNames(chord, inversion = "") {
  ...
  const uniquePitchClasses = [
    ...new Set(baseIntervals.map((interval) => (rootIndex + interval) % 12)),
  ];
  let orderedPitchClasses = rotateArray(
    uniquePitchClasses,
    toInversionIndex(effectiveInversion),
  );
  ...
  if (bass) {
    ...
    if (bassIndex !== undefined) {
      orderedPitchClasses = [
        bassIndex,
        ...orderedPitchClasses.filter((item) => item !== bassIndex),
      ];
    }
  }
...
export function getChordVoicingKeyIndexes(chord, inversion = "") {
  ...
  let orderedIntervals = rotateArray(baseIntervals, inversionSteps).map(
    (item) => ((item % 12) + 12) % 12,
  );
  ...
  if (bass) {
    ...
    if (bassIndex !== undefined) {
      ...
      orderedIntervals = [
        bassInterval,
        ...orderedIntervals.filter((item) => item !== bassInterval),
      ];
    }
  }
